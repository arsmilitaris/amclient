...
impl GameConsumer {
    ...
    pub fn recv(mut self) -> (String, String) {
        
        let mut message = (String::from(""), String::from(""));
		...
		for ms in self.c.poll().unwrap().iter() {
			for n in (ms.messages().len() - 1)..ms.messages().len() {
				...
				//println!("{}", n);
				message = (
					String::from_utf8_lossy(ms.messages()[n].key).into(),
					String::from_utf8_lossy(ms.messages()[n].value).into(),
				);
			}
		};
    ...
    pub fn recv_all(mut self) -> Vec<(String, String)> {
		
		let mut message_list: Vec<(String, String)> = Vec::new();
		let mut message = (String::from(""), String::from(""));
        
		for ms in self.c.poll().unwrap().iter() {
			for n in 0..ms.messages().len() {
				//println!("{:?}", ms.messages()[n]);
				//println!("{}", n);
				message = (
					String::from_utf8_lossy(ms.messages()[n].key).into(),
