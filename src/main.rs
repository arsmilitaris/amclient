...
//use tracing_appender::WorkerGuard;
//
//use bevy::log::Level;
//
//#[derive(Resource)]
...
//impl Resource for MyWorkerGuard {
//    fn name() -> &'static str {
//        "MyWorkerGuard"
//    }
//
//    fn from_resource(resource: &Resources) -> Option<Self> {
//        resource.get::<Self>().cloned()
//    }
//
//    fn set_to_resource(resources: &mut Resources, resource: Self) {
...
//            let file_appender = tracing_appender::rolling::never("", "amclient.log");
...
//			app.insert_resource(WorkerGuardResource(worker_guard)); // have to keep this from being dropped
//
...
fn custom_panic_hook(info: &std::panic::PanicInfo) {
    ...
    info!("DEBUG: Panic occurred: {:?}", info);
    
    let mut file = File::create("amclient.log").expect("Failed to create log file");

    // Write the panic information to the log file
    writeln!(file, "Panic occurred: {:?}", info).expect("Failed to write to log file");
}
...
//        // for file name
//        let dt = Local::now();
//        let path: PathBuf = dt.format("amclient.log").to_string().into();
//
